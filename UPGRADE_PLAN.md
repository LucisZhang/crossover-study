...
## 8b. Phase 8 — Crossover Study, Part II *(APPROVED 2026-08-17 — owner sign-off recorded; execute in §8 task order T8-1 → T8-4)*

**Why this phase exists.** Phase 4 answered the headline question in the negative: pop-trailing-12m (TEST NDCG@10 0.00540) beats ALS (0.00275), item-kNN (0.00095), and content (0.00089) at every history-depth segment; the ALS deficit narrows monotonically (−0.00256 at 1–4 → −0.00146 at 20+) but never crosses, and the router collapsed to n\*=∞. Two things about that result are not yet discharged. First, the churn diagnosis — that 2023 TEST purchases land on post-cutoff items a TRAIN-frozen model structurally cannot rank — was *derived from the pattern, never directly measured*; `docs/case_study.md` line 178 concedes the result "is a statement about *this* regime," and that "a category with slower catalog turnover, or a model with access to post-cutoff signal could all move it." Second, and more sharply, the comparison was **unfair in one axis nobody tested**: popularity got a trailing-12-month recency window while every CF arm was static all-history. This phase measures the churn and closes the recency-asymmetry confound before the null is presented as robust. Literature context: popularity is a known-strong baseline under global temporal split + full-catalog eval (Ferrari Dacrema et al., RecSys'19; Ji et al., TOIS'23); the canonical profile-length crossover study is Kluver & Konstan (RecSys'14).

**Invariant compliance (this whole phase).** Frozen TEST is respected by treating each new arm as a final reported run, not an iteration target: tuning and selection happen on VAL, and each selected arm gets **exactly one preregistered TEST evaluation** — stated here as an explicit assumption, since the invariant forbids TEST for iteration, not for one final scored run per arm (this is the same posture Phases 3–4 used). All stochastic steps carry recorded seeds (ALS init and bootstrap resamples reuse the frozen 3-seed set); `results/runs.jsonl` stays append-only with config/git/manifest/snapshot hashes; ranking stays full-catalog (TRAIN-seen items excluded, no sampled negatives); lane discipline holds — **both new arms are classical** (co-occurrence kNN and matrix-factorization ALS), no training/fine-tuning of deep models; no raw-data redistribution.

**T8-1 — Measure the churn: catalog-learnability regime map (effort S–M, ~0.5–1 day).** 2D stratification of existing TEST results: user TRAIN-history depth × target-item **learnability at the train cutoff** (item TRAIN support bucketed zero / low / high interaction count; first-seen date; recency of last TRAIN interaction). Pure Spark aggregation plus recomposition of the already-computed per-user TEST scores — **no retraining, no new TEST model runs.**
✅ *Accept:* logged to `results/runs.jsonl` with seeds/hashes: (a) the fraction of TEST ground-truth hits on items absent/sparse in TRAIN (the missing receipt for the churn diagnosis); (b) per-cell population share and paired ALS-vs-pop-t12m deltas with user-bootstrap CIs; (c) max attainable recall for factor models given item coverage per cell. An `EXPERIMENT_LOG.md` entry converts the churn diagnosis from *derived* to *measured*. **Guard recorded explicitly:** if TRAIN/TEST catalog overlap turns out near-total, the churn diagnosis is wrong and T8-2's design must be revisited before proceeding.

**T8-2 — Recency-matched personalization: the fairness test (effort M, ~2–4 days).** Exactly two new arms, both classical and in-lane: (1) **item-kNN with co-occurrence computed on trailing-12m TRAIN interactions only**; (2) **implicit ALS with time-decayed confidence weights** — the existing rank=128 / reg=0.01 / α=10 / iter=25 / 3-seed config held fixed, with **only the decay half-life tuned** (preregister a 2–3-value half-life grid, select on VAL). **Preregistration in `EXPERIMENT_LOG.md` BEFORE any TEST run:** hypotheses, half-life grid, VAL selection rule, and decision rules for both outcomes. Then **one TEST evaluation per selected arm**, with the segment breakdown identical to the existing exhibits.
✅ *Accept:* preregistration entry timestamped before the first TEST run; two VAL selection records + two final TEST records in `runs.jsonl` with CIs and paired deltas vs pop-t12m, per segment. Both outcomes ship: crossover appears → the headline question gets a positive answer ("personalization needs history **and** freshness"); still no crossover → the null becomes robust to the recency asymmetry.

**T8-3 — Extend the depth axis (effort S, ~0.5 day).** Recompose existing per-user TEST scores into deeper buckets (20–49, 50–99, 100+; boundaries fixed from TRAIN-history counts **before** looking at per-bucket outcomes). Labeled **exploratory/derived** (motivated by the observed narrowing), not confirmatory.
✅ *Accept:* per-bucket deltas with user-bootstrap CIs in `runs.jsonl`; user counts and CI widths reported honestly — thin buckets are expected and disclosed as such.

**T8-4 — MovieLens-32M regime contrast (effort L, ~1.5–3 weeks) — CONDITIONAL: execute only if T8-2 confirms the null.** Gate is explicit on T8-2's outcome. ML-32M (32.0M ratings, 200,948 users, 87,585 movies, timestamps to Oct 2023, ~240MB, research license — no redistribution, same posture as the current data; cite Harper & Konstan 2015): its own frozen preregistered temporal split (mirror design, TEST = 2023), own contracts, the same model ladder, the same full-catalog + bootstrap-CI harness. Framed as **external replication / regime contrast** ("crossover exists only when the catalog holds still"), **not causal proof** — explicit-rating movie data changes several variables at once; disclose the MovieLens timestamp caveat (rating-entry time, backfilled catalogs; cite Sun et al., arXiv:2307.09985). Rejected alternatives: Last.fm LFM-1b/2b (license withdrawn), Netflix Prize (withdrawn), denser Amazon categories still too sparse (Kindle Store ~4.6 ratings/user mean).
✅ *Accept:* if the gate opens — ML-32M manifest + contracts + frozen split committed; full model ladder scored with CIs and the crossover chart rendered under both regimes; the "regime contrast, not causal proof" framing and timestamp caveat written into the evidence sections. If the gate stays closed, this task is not started.

**Follow-on doc/demo work (part of Phase 8 acceptance, not separate scope).** Update `docs/case_study.md` §6–7 to replace the derived churn language with the measured regime map and to record the fairness-test outcome; discharge the line-178 caveats explicitly. Extend the crossover explorer (§9.1) with the recency-matched arms and the deeper depth buckets, each wired to receipts. If T8-4 runs, add a regime-contrast view.

**Cut-order placement (proposed §12 integration; the table itself is edited only on approval).** These tasks enter the cut order **above** the current Phase 7 stretch items — i.e., first to cut: T8-4 (conditional and by far the largest) → T8-3 (cheapest and most disposable, but also the first *keep* candidate if any of Phase 8 survives) → T8-2 → T8-1. Mirroring the existing §12 ALS line: **a T8-2 null is a publishable outcome, not a failure** — "personalization stays behind popularity even after the recency asymmetry is removed" is a stronger, more defensible claim than the un-controlled null it replaces.

**Total added effort:** ~3–5 days for T8-1/2/3; T8-4 adds ~1.5–3 weeks only if its gate opens.

## 8c. Phase 9 — Null repin & regime contrast *(APPROVED 2026-08-19 — owner-directed; supersedes the T8-4 gate closure; execute T9-1 → T9-4)*

**Why this phase exists.** The Phase 8 close-out declared "crossover MET" on the strength of item-kNN-t12m winning 5 of ~40 regime-map cells. That verdict is real but fragile: ~40 cells × 2 arms ≈ 80 uncorrected tests per metric, so α=0.05 alone predicts ~4 false positives; the dual-metric guard (NDCG@10 + Recall@20 both significant) is partial protection because the two metrics are highly correlated on the same cells; the winning arm is the globally weakest model in the ladder (TEST NDCG@10 0.000301 vs pop-t12m 0.005404); the absolute deltas are +0.0003–0.0007 and the 5 cells carry per-cell TEST GT shares of 0.69–2.16% (summing to 7.34% across cells per run `20260818T072256Z-3f3530a`; the two axes overlap, so the union of interactions is smaller still). *[Correction 2026-08-19: an earlier draft of this paragraph said "7.64% of TEST GT interactions" — that figure traces to no record and is superseded by the per-cell shares above.]* Two consequences are not acceptable to leave standing: (a) the public headline dresses a clean, mechanism-diagnosed null (no history-depth crossover anywhere; optimal policy is *no routing*) as a weak positive, and (b) the T8-4 regime contrast — the one experiment that would convert "null on one high-churn dataset" into "catalog churn is the controlling variable" — was skipped *because of* this fragile verdict. This phase repins the narrative to the robust null and reopens T8-4 as a confirmatory contrast. The cell clustering in the 91–365d staleness bucket remains a legitimate mechanism observation (pop-t12m's trailing window under-serves stale items); it is demoted from headline to footnote, not deleted.

**Invariant compliance (this whole phase).** The frozen Amazon TEST split is untouched — T9-1/T9-2 are doc-and-log work with **zero new Amazon runs**; ML-32M gets its own frozen preregistered split per the T8-4 spec. `results/runs.jsonl` and `EXPERIMENT_LOG.md` stay append-only: the T8-2 verdict is downgraded by **superseding entries**, never edits. All stochastic steps carry recorded seeds; ranking stays full-catalog with bootstrap CIs; lane discipline holds (classical models + MiniLM content arm only, no fine-tuning); ML-32M is research-licensed and never redistributed (cite Harper & Konstan 2015).

**T9-1 — Null-first narrative repin (effort S, ~0.5–1 day; docs/demo only, no new runs).** Rewrite the headline claim across `docs/case_study.md`, `README.md`, and the demo copy to lead with the robust findings: (1) no history-depth crossover exists on Amazon Electronics — ALS, item-kNN, and pure content lose to pop-t12m at every depth segment with CIs excluding zero, before and after recency matching; (2) the measured mechanism is catalog churn (41.11% of TEST GT items have zero/low TRAIN support, run `20260817T095926Z-633d454`); (3) the one effective arm is content–pop blend α=0.3 (+0.000322 NDCG@10, ≈+6% relative, CI excludes zero); (4) the fitted routing policy is **n\*=∞ — the optimal policy is no routing**, a decision-relevant negative that spares the segmentation infrastructure. Demote the 5-cell kNN-t12m result to a mechanism footnote and add the multiplicity disclosure verbatim in spirit: ~80 uncorrected tests, ~4 expected false positives at α=0.05, clustering + dual-metric consistency argued as partial (not corrective) evidence. Append the `EXPERIMENT_LOG.md` supersession entry downgrading the T8-2 verdict language from "crossover MET" to "local regime-cell wins under uncorrected multiplicity; the global history-depth crossover null is robust." Update the §10 evidence sections ("How this was verified" / "What this does not prove") to match.
✅ *Accept:* no metric value changes anywhere; every changed claim still traces to an existing `runs.jsonl` run_id; `make reproduce-headline` still passes byte-exact; the supersession entry is appended (log history shows no rewrites); §10 sections updated.

**T9-2 — Reopen the T8-4 gate (effort XS; decision record).** Append an `EXPERIMENT_LOG.md` entry recording: the 2026-08-19 T8-4 skip was gated on the now-downgraded verdict; the owner reopens T8-4 as approved scope. The §8b T8-4 spec stands as written with **one amendment**: the preregistration must now include an explicit multiplicity policy — the **primary confirmatory test is the per-depth-segment crossover on the history axis** (5–8 tests, mirroring the Phase 4 exhibits); regime-map cells are **secondary**, corrected with Benjamini–Hochberg at FDR 0.05 across all cells per arm.
✅ *Accept:* decision entry appended before any ML-32M work begins; the amendment text is quoted in the entry.

**T9-3 — Execute T8-4: ML-32M regime contrast (effort L, ~1.5–3 weeks; per §8b spec + T9-2 amendment).** Staged:
- **T9-3a — Data.** Download ML-32M with SHA-256s in `data/MANIFEST.md` (disk gate per §5 first); bronze → silver → gold through the same contract engine (new `contracts/*.yaml` per table); freeze `configs/splits_ml32m.yaml` (mirror design, TEST = 2023, dates fixed before any model sees them); rerun the T8-1 churn methodology to produce the churn-contrast statistic (Amazon 41.11% vs ML-32M x%) — this number is the hinge of the whole contrast and is computed **before** any model evaluation.
- **T9-3b — Eval.** Preregister in `EXPERIMENT_LOG.md` before the first TEST run: hypotheses, VAL selection rules, half-life/window grids, and the T9-2 multiplicity policy. Run the same model ladder: pop (windowed variants), item-kNN (± trailing window), ALS (± time-decay), content arm on title+genres+tags via the existing MiniLM recipe (if metadata proves unusable, disclose as a limitation rather than substituting), and the blend. Tune on VAL only; **one TEST evaluation per selected arm**; headline config at 3 seeds.
- **T9-3c — Verdict.** Render the crossover chart under both regimes; append the verdict entry: either "crossover at depth n\* on ML-32M with CIs" or a robust double-null. **Both outcomes are publishable** — "crossover appears when the catalog holds still" and "popularity dominates even there" are each defensible headlines; the framing stays *regime contrast, not causal proof*, with the MovieLens timestamp caveat per §8b.
✅ *Accept:* §8b T8-4 acceptance criteria met (manifest + contracts + frozen split committed; full ladder with CIs; both-regime chart; caveats in evidence sections), plus: preregistration entry timestamped before the first TEST run; a recorded reproduce command (`make reproduce-ml32m` or documented equivalent) pinned to the ML-32M Iceberg snapshot.

**T9-4 — Cross-dataset synthesis & publication (effort M, ~1–2 days).** New case-study section presenting churn as the controlling variable: side-by-side regime table (churn %, per-depth deltas, crossover verdict per dataset), the "no routing is the optimal routing" policy discussion, and the regime-contrast caveats. Extend the crossover explorer with a dataset toggle or contrast panel wired to receipts. Draft the null-first, business-decision-led external case-study copy for the portfolio site as a deliverable file under `docs/` (updating the site itself is out of repo scope).
✅ *Accept:* every published number traces to `results/runs.jsonl`; both reproduce targets pass; §10 sections cover ML-32M; the site-copy draft exists and cites run_ids for every figure it quotes.

**Cut-order placement.** T9-1 and T9-2 are **never cut** — they are corrections, cost under a day combined, and carry the project's integrity. Then first to cut: T9-4 demo polish → T9-3 (the whole contrast, as one unit). A cut T9-3 leaves a fully honest single-dataset null study; that is the §12-sanctioned publishable outcome, not a failure.

**Total added effort:** ~1 day for T9-1/T9-2; T9-3 ~1.5–3 weeks; T9-4 ~1–2 days.
...
| T8-2 recency-matched arms still lose to pop-t12m | Publishable — a T8-2 null is an outcome, not a failure: "personalization stays behind popularity even after the recency asymmetry is removed" is a stronger, more defensible claim than the un-controlled null it replaces (§8b). |
...
**Cut order if scope must shrink (cut from the top):** 1) T8-4 ML-32M regime contrast (conditional and by far the largest) → 2) T8-3 deep depth buckets (cheapest and most disposable — but also the first *keep* candidate if any of Phase 8 survives) → 3) T8-2 recency-matched arms → 4) T8-1 regime map → 5) Phase 7 stretch items → 6) live in-browser semantic search (precomputed queries instead) → 7) late-data upsert + compaction exhibits (keep snapshot-pinned repro) → 8) item-kNN baseline (keep popularity variants) → 9) confidence-weighting variant → 10) Iceberg → partitioned Parquet (last infrastructure resort). **Never cut:** full-dataset ingestion with the reconciliation waterfall, contracts + quarantine, temporal splits, full-catalog ranking metrics with CIs, the popularity baseline, the crossover/segment analysis, the routing policy, the two verification sections, and the provenance paragraph — they are the identity of the piece.
