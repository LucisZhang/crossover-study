...
**TEST is touched once per model.** Before every TEST campaign, a pre-declaration was written and committed to `EXPERIMENT_LOG.md` *first*: the arms, the comparisons, the acceptance gate, and an explicit no-return clause — whatever TEST shows, there is no going back to the VAL grid. Two such pre-declarations exist (Phase 3's T7 for ALS, Phase 4's T15 for content/blend/hybrid), both timestamped ahead of their results **[measured, `EXPERIMENT_LOG.md`]**. This is the part of the protocol that makes a negative finding worth reading.
...
The comparison above had one asymmetry: popularity got a trailing-12-month window while every CF arm was static all-history. Phase 8 re-ran the fight recency-matched **[measured]**: time-decayed ALS (half-life selected on VAL: 365d) and item-kNN with its co-occurrence restricted to the same trailing-12-month window. ALS-decay beat static ALS on VAL (+25% relative NDCG@10) but still loses to pop-t12m globally (3-seed TEST 0.003734 ± 0.000018 vs 0.005404) and at every depth — the depth-level null is robust to the fairness objection, now by measurement rather than argument. Windowed item-kNN collapses globally (0.000301, the worst arm in the lab). But recomposed through the T8-1 regime map, kNN-t12m beats pop-t12m **with 95% CIs excluding zero on both NDCG@10 and Recall@20** in five cells — low-support items at depths 5-9 and 20+, and items last seen 91–365 days before cutoff at depths 1-4, 5-9 and 10-19 (each cell 0.7–2.2% of TEST ground-truth mass) **[measured, runs [`20260818T072256Z-3f3530a`](demo/index.html#receipt-20260818T072256Z-3f3530a), [`20260818T072211Z-3f3530a`](demo/index.html#receipt-20260818T072211Z-3f3530a)]**. Where popularity's own window under-ranks the stale catalog, even a globally weak personalized arm finds signal popularity cannot. The cells are defined by ground-truth item properties a serving-time router cannot observe, so this is a diagnosis of popularity's blind spot, not yet a routable policy — the full protocol, preregistration and multiplicity disclosure are in `EXPERIMENT_LOG.md` (2026-08-18).
...
- **One config per run, append-only log with git SHAs.** 60 records in `results/runs.jsonl` across evals, paired deltas, ANN receipt, ops, lineage, reproduce, the DQ export, and the derived policy grid — each carrying run_id, git SHA and dirty flag, config path and hash, dataset manifest hash, and Iceberg snapshot IDs. **[measured]**
...
- **Experiment log published, failures included.** `EXPERIMENT_LOG.md` carries dated hypothesis → result → verdict entries including the rejected kNN neighbor-list hypothesis, the ALS negative, the content-alone rejection, the collapsed router, the sub-expectation ANN overlap, and the aborted shopper-curation rule. **[measured]**
