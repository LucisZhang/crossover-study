...
Two things are visible. First, popularity's advantage genuinely erodes with depth — it is nearly twice as strong for cold-start users as for the `20+` bucket. Second, nothing catches it. The ALS-minus-popularity deficit narrows monotonically (−0.0026 → −0.0015 as depth increases) but the sign never flips, and the CI at `20+` is [−0.0021, −0.0008]: still comfortably below zero **[measured]**.
