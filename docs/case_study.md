<!--
  Batch Recsys Lab — case study, final copy (Phase 6 T37, 2026-08-09).
...
> **"How much history does a user need before personalization beats popularity?"** Batch Recsys Lab ingests 43.9M Amazon Electronics reviews through a contract-checked Spark + Iceberg lakehouse on a single 16GB laptop, evaluates popularity, item-kNN, ALS, and semantic-content retrieval under temporal splits with full-catalog ranking and bootstrap CIs — and measures the answer: **history is the wrong axis on its own. Personalization needs history *and* freshness — and where freshness is popularity's weakness, even a weak personalized arm finds signal.** No depth of history is enough by itself: pure personalization never beats recency-weighted popularity at any observed depth, and the only arm that beats it globally is popularity itself, gently re-ranked by a normalized semantic-content blend (+5.96% relative NDCG@10, 1,000-resample paired CIs cleanly separating it from zero). The freshness half is measured, not argued: 41.1% of 2023 TEST purchase mass lands on items with zero or near-zero training support, capping any TRAIN-frozen model at 65.5% recall before a single modeling choice is made. And in the pocket where popularity's own trailing-12-month window goes blind — stale, thinly-supported items — a recency-matched item-kNN that is the *worst global arm in the lab* beats popularity with CIs excluding zero on both metrics, in five regime-map cells. The depth-level answer stands; the crossover lives on the freshness axis instead (§6).
...
So the honest headline is a negative finding with two positives attached. On 2023 Amazon Electronics, with training frozen at 2022-06-30, no personalization arm tested here beats knowing what is currently popular — not globally, and not at any history depth, including after Phase 8 removed the recency asymmetry that made the first comparison unfair. Content similarity earns its place only as a gentle re-ranker on top of that. The second positive is finer-grained and arrived last: once the catalog is split by how *learnable* each purchased item was at the training cutoff, there is a measured pocket where popularity is the arm that cannot see, and a personalized arm wins there with CIs excluding zero (§6). The machinery below — lakehouse, contracts, protocol — is what makes both the negative and that pocket trustworthy enough to publish.
...
## 6. The crossover that never came — and the one that did
...
Two things are visible. First, popularity's advantage genuinely erodes with depth — it is nearly twice as strong for cold-start users as for the `20+` bucket. Second, at history-depth granularity nothing catches it (the Phase 8 addendum below records the one finer-grained exception). The ALS-minus-popularity deficit narrows monotonically (−0.0026 → −0.0015 as depth increases) but the sign never flips, and the CI at `20+` is [−0.0021, −0.0008]: still comfortably below zero **[measured]**.
...
That is the preregistered crossover criterion, met — on exactly the axis the hypothesis named (items popularity's own window under-ranks), by the arm the hypothesis did not favor. **Where freshness is popularity's weakness, even a weak personalized arm finds signal.** Three things keep it honest. It is a minority pocket: each cell is 0.7–2.2% of TEST ground-truth mass and the cells overlap across the two axes. Multiplicity is uncorrected: roughly 40 cells × 2 arms were tested under the preregistered "any cell" rule, with no correction applied — the clustering (two related regions, three consecutive depth bands) and the agreement of both metrics in every cell argue against pure chance, but they are an argument, not a correction. And the cells are keyed to properties of the *ground-truth item* — its TRAIN support and recency — which a serving-time router cannot observe. This is a **measured diagnosis of popularity's blind spot, not a routable policy** (§7). The full protocol, preregistration, lineage exception and multiplicity disclosure are in `EXPERIMENT_LOG.md` (2026-08-17 / 2026-08-18).
...
- *"A category with slower catalog turnover could move it."* — **Still stands, and is now the largest open question in the study.** The external contrast that would have tested it (T8-4, MovieLens-32M) was gated on T8-2 confirming the null; T8-2 found a crossover, so the gate closed and T8-4 was formally skipped by the rule as written (`EXPERIMENT_LOG.md`, 2026-08-18). "The crossover exists only when the catalog holds still" remains an untested framing, not a finding of this lab.
...
- *"At the depths its users actually reach, the crossover does not occur."* — **Holds at history-depth granularity, and superseded below it.** T8-3 extended the depth axis to 20-49 / 50-99 / 100+ and found no crossover that survives a change of metric; the crossover that does survive is on the freshness axis, inside the regime-map cells above.
- *"This is a statement about this regime, not about collaborative filtering."* — **Still stands, unchanged.** One category, one snapshot, one train/test cutoff. Nothing in Phase 8 extends the claim beyond it; Phase 8 only replaced guesses about the regime with measurements of it.
...
**What Phase 8 changes here: nothing yet, and that is a finding too.** Phase 8 produced the first *measured* reason to want a non-trivial router — a pocket of the catalog where popularity is structurally blind and a personalized arm wins with CIs excluding zero (§6). It also produced the reason that pocket cannot be routed to today: the cells are defined by the TRAIN support and TRAIN recency of the *item the user is about to buy*, and a serving-time router does not know that item. Routing on the winning cells would require a proxy — some observable, user-side signal that this shopper is working the stale, thinly-supported corner of the catalog — fitted on VAL and given its own preregistered TEST run. That is out of Phase 8's declared scope, and it is stated as an open lead rather than smuggled in as a result.
...
- **The measured crossover is a small, uncorrected, non-routable slice.** Five regime-map cells beat popularity with CIs excluding zero on both metrics, and each is 0.7–2.2% of TEST ground-truth mass with per-cell deltas in the fourth decimal. Roughly 40 cells × 2 arms were tested under the preregistered any-cell rule with **no multiple-comparison correction**; coherence across two metrics and adjacent depth bands is the argument, and coherence is not a correction. The cells are keyed to the TRAIN support and recency of the ground-truth item, which no serving-time router can observe — so this is a diagnosis of where popularity goes blind, not a policy anyone can deploy from these numbers.
- **No external replication; the low-churn contrast was never run.** T8-4 (MovieLens-32M, a deliberately different catalog-churn regime) was gated on Phase 8 confirming the null. It did not, so the gate closed and T8-4 was skipped by the preregistered rule. The attractive framing "the crossover exists only when the catalog holds still" is therefore an untested hypothesis in this lab, not a result of it, and every Phase 8 number remains a single-dataset, single-cutoff measurement.
